...
\begin{itemize}
    ...
    \item Byzantine fault tolerant consensus (QBFT/IBFT-style) across four independently-operated validators---the electoral authority and three political parties---requiring a quorum of three signatures to commit a block
...
\begin{enumerate}
    ...
    \item More than $2n/3$ validator nodes are honest---for the deployed set of $n=4$, at least $3$, so the system tolerates one faulty or unreachable validator
...
\hline
State rollback & Revert blockchain state & Single-slot BFT finality (no reorgs) \\
\hline
...
\begin{enumerate}
    ...
    \item \textbf{Block Commit}: BFT consensus finalizes the state change once three of the four validators have independently re-executed and signed the block

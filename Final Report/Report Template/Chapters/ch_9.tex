...
\begin{enumerate}
    ...
    \item \textbf{Multi-party block production}: We distributed the authority to produce blocks across four independently-operated validators---the electoral authority and three political parties---under a QBFT/IBFT-style protocol requiring three signatures to commit. Each validator re-executes a proposed block before signing it, so no single operator can halt the election or unilaterally decide its contents, and falsifying history requires three of the four to collude.
...
\hline
3 & Build a custom gasless blockchain embedding the EVM & Achieved & Go blockchain with BFT consensus across four validators (3-of-4 quorum) and JSON-RPC; executes same bytecode as Hardhat \\
\hline
...
\subsection{Consensus Trust and Membership}

Byzantine fault tolerance bounds the trust required rather than eliminating it, and the bound should be stated precisely: a quorum of three of the four validators can control transaction ordering and omit ballots. What even a colluding majority cannot do is finalize a block with an invalid state root, because every honest validator re-executes before signing---so the residual attack surface is censorship, not fabrication, and censorship of a determined voter is externally observable.

Three further limitations follow from the design:

\begin{itemize}
    \item \textbf{The validator set is fixed at configuration.} There is no on-chain membership management, because changing the set requires deciding at which block the change takes effect---a decision that affects the commit certificates of blocks on either side. Adding or replacing a validator is therefore a coordinated restart of the whole cluster.
    \item \textbf{Losing two validators halts the election.} This is correct by construction rather than a defect: two of four must not be able to decide an election between themselves. It is a deliberate exchange of liveness for safety, on the reasoning that a halted election can be resumed whereas a forged one cannot be undone.
    \item \textbf{Key custody in this deployment is centralized.} All four consensus signing keys were generated by one operator for the prototype. The protocol does not assume this: in a real deployment each party would generate its key on its own hardware and publish only the corresponding address, which the configuration already accommodates. The distinction matters because the cryptographic guarantee is genuinely four-party while the present \emph{custody} is not.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Dynamic Validator Membership}: Move the validator set on-chain so that a party can be added, replaced, or removed without a coordinated restart of the cluster. The substantive design problem is not the mechanism but the epoch boundary: a commit certificate is only meaningful relative to a known set, so the protocol must define unambiguously which set governs which block, and an auditor verifying historical certificates must be able to reconstruct that mapping from the chain alone.
